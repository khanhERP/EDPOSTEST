...
import { eq, and, gte, lte, desc, asc, sql, like, or, not } from "drizzle-orm";
import { convertDatesInBody } from "./vite";
import type {
  Product,
  Category,
  Order,
...
export function registerRoutes(app: express.Application): Promise<any> {
  return new Promise(async (resolve) => {
    ...
    // API endpoint to close popup by ID
    app.post("/api/popup/close", async (req, res) => {
      try {
        const { popupId, targetClientId } = req.body;
        ...
        console.log('Sending close popup signal:', signal);

        // Send signal to all connected clients or specific client
        let sentCount = 0;
        for (const [clientId, ws] of popupSignalClients.entries()) {
          if (ws.readyState === WebSocket.OPEN) {
            // Send to specific client if targetClientId is provided, otherwise send to all
            if (!targetClientId || clientId === targetClientId) {
        ...
        res.json({ 
          success: true, 
          message: `Close popup signal sent to ${sentCount} client(s)`,
          popupId,
          targetClientId: targetClientId || 'all clients'
        });
      } catch (error) {
        console.error("Error sending close popup signal:", error);
        res.status(500).json({
          error: "Failed to send close popup signal",
          details: error.message,
        });
      }
    });
...
async function initializeSampleData() {
  ...
  // In a real application, this would involve seeding the database
  console.log("Initializing sample data...");

  // Example: Check if categories table is empty and seed if necessary
  const existingCategories = await db.select().from(categories);
  if (existingCategories.length === 0) {
    console.log("Seeding categories...");
    await db.insert(categories).values([
      { name: "Appetizers", icon: "fas fa-star" },
      { name: "Main Courses", icon: "fas fa-utensils" },
      { name: "Desserts", icon: "fas fa-ice-cream" },
      { name: "Beverages", icon: "fas fa-coffee" },
    ]);
  }

  // Example: Check if products table is empty and seed if necessary
  const existingProducts = await db.select().from(products);
  if (existingProducts.length === 0) {
    console.log("Seeding products...");
    const categoryAppetizer = await db.select().from(categories).where(eq(categories.name, "Appetizers")).limit(1);
    const categoryMainCourse = await db.select().from(categories).where(eq(categories.name, "Main Courses")).limit(1);
    const categoryDessert = await db.select().from(categories).where(eq(categories.name, "Desserts")).limit(1);
    const categoryBeverage = await db.select().from(categories).where(eq(categories.name, "Beverages")).limit(1);

    await db.insert(products).values([
      { name: "Garlic Bread", sku: "GB001", price: "4.99", stock: 50, categoryId: categoryAppetizer[0]?.id || 1, productType: 1, trackInventory: true, taxRate: "8.00" },
      { name: "Caesar Salad", sku: "CS002", price: "7.50", stock: 30, categoryId: categoryAppetizer[0]?.id || 1, productType: 1, trackInventory: true, taxRate: "8.00" },
      { name: "Spaghetti Carbonara", sku: "SC003", price: "15.99", stock: 20, categoryId: categoryMainCourse[0]?.id || 2, productType: 1, trackInventory: true, taxRate: "8.00" },
      { name: "Grilled Salmon", sku: "GS004", price: "18.50", stock: 15, categoryId: categoryMainCourse[0]?.id || 2, productType: 1, trackInventory: true, taxRate: "8.00" },
      { name: "Chocolate Cake", sku: "CC005", price: "6.50", stock: 25, categoryId: categoryDessert[0]?.id || 3, productType: 1, trackInventory: true, taxRate: "8.00" },
      { name: "Cheesecake", sku: "CK006", price: "7.00", stock: 20, categoryId: categoryDessert[0]?.id || 3, productType: 1, trackInventory: true, taxRate: "8.00" },
      { name: "Coffee", sku: "CO007", price: "3.00", stock: 100, categoryId: categoryBeverage[0]?.id || 4, productType: 1, trackInventory: false, taxRate: "8.00" },
      { name: "Orange Juice", sku: "OJ008", price: "3.50", stock: 80, categoryId: categoryBeverage[0]?.id || 4, productType: 1, trackInventory: false, taxRate: "8.00" },
      { name: "Combo Meal 1", sku: "CM009", price: "20.00", stock: 10, categoryId: categoryMainCourse[0]?.id || 2, productType: 3, trackInventory: true, taxRate: "8.00" }, // Example combo
    ]);
  }

  // Example: Check if employees table is empty and seed if necessary
  const existingEmployees = await db.select().from(employees);
  if (existingEmployees.length === 0) {
    console.log("Seeding employees...");
    await db.insert(employees).values([
      { name: "John Doe", email: "john.doe@example.com", role: "Manager", isActive: true },
      { name: "Jane Smith", email: "jane.smith@example.com", role: "Staff", isActive: true },
      { name: "Peter Jones", email: "peter.jones@example.com", role: "Staff", isActive: false },
    ]);
  }

  // Example: Check if customers table is empty and seed if necessary
  const existingCustomers = await db.select().from(customers);
  if (existingCustomers.length === 0) {
    console.log("Seeding customers...");
    await db.insert(customers).values([
      { name: "Alice Wonderland", email: "alice@example.com", phone: "123-456-7890", customerId: "CUST001", totalSpent: 150.75, visitCount: 5, status: "active" },
      { name: "Bob The Builder", email: "bob@example.com", phone: "987-654-3210", customerId: "CUST002", totalSpent: 210.50, visitCount: 8, status: "active" },
    ]);
  }
