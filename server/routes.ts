...
// Get tables with calculated order totals - optimized endpoint
app.get("/api/tables-with-totals", async (req: TenantRequest, res) => {
  try {
    ...
    const tablesWithTotals = tablesResult.map(table => {
      // Find all active orders for this table
      const tableOrders = activeOrdersWithItems.filter(item => item.tableId === table.id);
      ...
      const latestOrder = ordersArray.sort(
      ...
      if (latestOrder.items && latestOrder.items.length > 0) {
        latestOrder.items.forEach((item: any) => {
          ...
          // Calculate tax using same logic as table-grid
          if (
      ...
      console.log(`💰 Table ${table.tableNumber} calculated total:`, {
        ...
        calculatedTax,
        calculatedTotal,
        storedTotal: latestOrder.storedTotal
      });
    ...
    console.log(`✅ Successfully processed ${tablesWithTotals.length} tables with calculated totals`);
    res.json(tablesWithTotals);

  } catch (error) {
    console.error("❌ Error fetching tables with totals:", error);
    res.status(500).json({ error: "Failed to fetch tables with totals" });
  }
});
