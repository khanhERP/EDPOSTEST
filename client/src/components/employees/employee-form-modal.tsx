...
export function EmployeeFormModal({ isOpen, onClose, mode, employee }: EmployeeFormModalProps) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: InsertEmployee) => apiRequest('/api/employees', { method: 'POST', body: JSON.stringify(data) }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: InsertEmployee) => 
      apiRequest(`/api/employees/${employee?.id}`, { method: 'PUT', body: JSON.stringify(data) }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/employees'] });
      toast({
        title: "성공",
