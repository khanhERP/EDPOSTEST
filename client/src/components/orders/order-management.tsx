...
function OrderSummaryDisplay({ orderDetailsCalculation, selectedOrder }: {
  orderDetailsCalculation: { subtotal: number; tax: number; total: number };
  selectedOrder: Order;
}) {
...
export function OrderManagement() {
  ...
  const handlePaymentClick = async (order: Order) => {
    ...
    try {
      ...
      console.log(
        `🧮 Order Payment: Calculating totals for ${orderItemsData.length} items`,
      );
  ...
  const handlePaymentMethodSelect = async (method: string, data?: any) => {
    ...
    if (method === "paymentCompleted" && data?.success) {
      ...
      try {
        ...
        console.log(
          "✅ Order Management: Payment completed, no automatic receipt display",
        );
      } catch (error) {
                    ...
                    <div className="space-y-3">
                      ...
                      {order.discount && Number(order.discount) > 0 && (
                        <div className="flex items-center justify-between text-xs text-red-600">
                          ...
                          <span>
                            -
                            {formatCurrency(Math.floor(Number(order.discount)))}
                          </span>
                        </div>
                      )}
            ...
            <ScrollArea className="max-h-[60vh]">
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between items-center">
                        ...
                        <Badge
                          variant={
                            getOrderStatusBadge(selectedOrder.status).variant
                          }
                        >
                ...
                <div className="space-y-3 bg-gray-50 p-4 rounded-lg">
                  <OrderSummaryDisplay 
                    orderDetailsCalculation={orderDetailsCalculation}
                    ...
                    <div className="flex gap-2 pt-4">
                      <Button
                        onClick={() => {
                          ...
                          const orderDiscount = Math.floor(
                            Number(selectedOrder.discount || 0),
                          );
                          const finalTotal = Math.max(
                            0,
                            baseTotal - orderDiscount,
                          );
                          ...
                          if (finalTotal < 0) {
                            console.error(
                              "❌ Invalid final total after discount:",
                              finalTotal,
                            );
                            toast({
                              title: "Lỗi",
                              description:
                                "Tổng tiền sau giảm giá không hợp lệ. Vui lòng kiểm tra lại.",
                              variant: "destructive",
                            });
                            return;
                          }
                          ...
                          console.log(
                            "💰 Order Details: Receipt preview created with discount details:",
                            {
                              ...
                              receiptExactDiscount:
                                receiptPreview.exactDiscount,
                            },
                          );
                          ...
                          console.log(
                            "✅ Order Management: Payment data prepared (matching table-grid):",
                            {
                          ...
                          if (typeof window !== "undefined") {
                            ...
                            console.log("💾 Stored discount data globally:", {
                              ...
                              orderExactDiscount:
                                paymentOrderData.exactDiscount,
                            });
                          }
                          ...
                          console.log(
                            "🚀 Order Management: Showing receipt preview modal",
                          );
                        }}
          ...
          <div className="space-y-6">
                ...
                {selectedOrder.discount &&
                  Number(selectedOrder.discount) > 0 && (
                    <div className="mt-1">
                      <p className="text-sm text-gray-600">
                        {t("tables.subtotal")}:{" "}
                        {formatCurrency(orderDetailsCalculation.total)}
                      </p>
                ...
                {selectedCustomer && (
                  <div className="mt-2 pt-2 border-t border-blue-200">
                    ...
                    {(() => {
                      ...
                      return (
                        customerPointsValue < finalTotal && (
                          <p className="text-sm text-orange-600 mt-1">
                            {t("orders.remainingAmount")}:{" "}
                            {(
                              finalTotal - customerPointsValue
                            ).toLocaleString()}{" "}
                            ₫
                          </p>
                        )
            ...
            <div className="space-y-3">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder={t(
                    "orders.pointsPaymentDialog.searchPlaceholder",
                  )}
                  value={searchTerm}
                  ...
                  {(() => {
                    ...
                    return customerPointsValue >= finalTotal ? (
                      <div className="text-green-600">
                        <p className="text-sm">
                          ✓ {t("orders.enoughpointstopay")}
                        </p>
                        <p className="text-sm">
                          {t("orders.use")}:{" "}
                          {Math.ceil(finalTotal / 1000).toLocaleString()}P
                        </p>
                        <p className="text-sm">
                          {t("orders.remaining")}:{" "}
                          {(
                            (selectedCustomer.points || 0) -
                            Math.ceil(finalTotal / 1000)
                          ).toLocaleString()}
                          P
                        </p>
                      </div>
                      ...
                      <div className="text-orange-600">
                        ...
                        <p className="text-sm">
                          {t("orders.additionalpaymentrequired")}:{" "}
              ...
              <div className="p-4 bg-gray-50 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="border-t pt-2 flex justify-between font-medium text-orange-600">
                    <span>{t("orders.additionalpaymentrequired")}:</span>
                    <span>
                      {mixedPaymentData.remainingAmount.toLocaleString()} ₫
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        setQrLoading(true);
                        const { createQRPosAsync, CreateQRPosRequest } =
