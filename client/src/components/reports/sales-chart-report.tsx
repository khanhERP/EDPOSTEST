...
export function SalesChartReport() {
  ...
  const renderEmployeeReport = () => {
    ...
    try {
      ...
      // Filter completed orders for employee analysis
      ...
      filteredTransactions.forEach((order: any) => {
        const employeeCode = order.employeeCode || "UNKNOWN";
        const employeeName = order.cashierName || "Chưa xác định";
        const orderTotal = Number(order.total || 0);
        ...
        employeeData[employeeCode].orderCount += 1;
        employeeData[employeeCode].revenue += orderTotal;
        employeeData[employeeCode].discount += orderDiscount;
        employeeData[employeeCode].totalTax += orderTax;
        employeeData[employeeCode].total += orderTotal;
        
        if (!employeeData[employeeCode].paymentMethods[paymentMethod]) {
          employeeData[employeeCode].paymentMethods[paymentMethod] = 0;
        }
        employeeData[employeeCode].paymentMethods[paymentMethod] += orderTotal;
      });

      const data = Object.values(employeeData);
      
      // Pagination
      const totalPages = Math.max(1, Math.ceil(data.length / employeePageSize));
                ...
                onClick={() => {
                  const dataWithSummary = [
                    ...data.map((item) => ({
                      ...
                      ...paymentMethodsArray.reduce((acc, method) => {
                        acc[getPaymentMethodLabel(method)] = formatCurrency(
            ...
            <div className="w-full">
              <div className="overflow-x-auto xl:overflow-x-visible">
                <Table className="w-full min-w-[1400px] xl:min-w-full">
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-center border-r bg-green-50 w-12">
                        STT
                      </TableHead>
                      <TableHead className="border-r bg-green-50 min-w-[120px]">
                        Mã NV
                      </TableHead>
                      <TableHead className="border-r bg-green-50 min-w-[200px]">
                        Tên nhân viên
                      </TableHead>
                      <TableHead className="text-center border-r bg-green-50 w-20">
                        Số đơn bán
                      </TableHead>
                      <TableHead className="text-right border-r bg-green-50 min-w-[120px]">
                        Thành tiền
                      </TableHead>
                      <TableHead className="text-right border-r bg-green-50 min-w-[120px]">
                        Giảm giá
                      ...
                      <TableHead className="text-right border-r bg-green-50 min-w-[150px]">
                        Tổng tiền
                      </TableHead>
                      <TableHead className="text-right border-r bg-green-50 min-w-[150px]">
                        Khách đã thanh toán
                      ...
                      {paymentMethodsArray.map((method) => (
                        <TableHead
                          key={method}
                          className="text-right border-r bg-blue-50 min-w-[120px]"
                        >
                          {getPaymentMethodLabel(method)}
                        </TableHead>
                      ))}
                  ...
                  <TableBody>
                    {paginatedData.length > 0 ? (
                      <>
                        {paginatedData.map((item, index) => (
                          <TableRow key={item.employeeCode}>
                            ...
                            <TableCell className="text-right border-r text-blue-600">
                              {formatCurrency(item.revenue - item.discount)}
                            </TableCell>
                            <TableCell className="text-right border-r text-purple-600">
                              {formatCurrency(item.totalTax)}
                            </TableCell>
                            <TableCell className="text-right border-r font-semibold">
                              {formatCurrency(item.total)}
                            </TableCell>
                            <TableCell className="text-right border-r font-semibold text-green-700">
                              {formatCurrency(item.total)}
                            </TableCell>
                            {paymentMethodsArray.map((method) => (
                              <TableCell
                        ...
                        <TableRow className="bg-gray-50 font-semibold border-t-2">
                          ...
                          <TableCell className="text-right border-r text-blue-600">
                            {formatCurrency(
                              data.reduce(
                                (sum, item) => sum + item.revenue - item.discount,
                                0
                              )
                            )}
                          </TableCell>
                          ...
                          <TableCell className="text-right border-r font-bold text-green-700">
                            {formatCurrency(
                              data.reduce((sum, item) => sum + item.total, 0)
                            )}
                          </TableCell>
                          {paymentMethodsArray.map((method) => (
                            <TableCell
                              key={method}
                              className="text-right border-r font-semibold"
                            >
                              {formatCurrency(
                                data.reduce(
                                  (sum, item) =>
                      ...
                      <TableRow>
                        <TableCell
                          colSpan={10 + paymentMethodsArray.length}
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-gray-500">
                  Hiển thị {Math.min(startIndex + 1, data.length)} -{" "}
                  {Math.min(endIndex, data.length)} của {data.length} nhân viên
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-500">
                    Trang {employeeCurrentPage} / {totalPages}
                  </span>
                  <button
      ...
      return (
        <div className="flex justify-center py-8">
          <div className="text-red-500">Có lỗi xảy ra khi tải dữ liệu nhân viên</div>
        </div>
      );
    }
  };
