    ...
    const renderCustomerReport = () => {
      ...
      const customerSales: {
        [customerId: string]: {
          ...
          orders: number;
          cancelledOrders: number; // Thêm số đơn đã hủy
          totalAmount: number;
          discount: number; // Default discount to 0
          revenue: number;
          status: string;
      ...
      filteredOrders.forEach((order: any) => {
        ...
        if (!customerSales[customerId]) {
          customerSales[customerId] = {
            ...
            orders: 0,
            cancelledOrders: 0, // Initialize cancelledOrders
            totalAmount: 0,
            discount: 0, // Default discount to 0
            revenue: 0,
      ...
      const cancelledOrders = orders.filter((order: any) => {
        ...
        return dateMatch && customerMatch && order.status === "cancelled";
      });

      cancelledOrders.forEach((order: any) => {
        const customerId = order.customerId || "guest";
        if (customerSales[customerId]) {
          customerSales[customerId].cancelledOrders += 1;
        }
      });
                ...
                onClick={() =>
                  exportToExcel(
                    data.map((item) => ({
                      ...
                      "Số đơn": item.orders,
                      "Số đơn đã hủy": item.cancelledOrders, // Export cancelled orders
                  ...
                  <TableBody>
                    ...
                    {data.length > 0 && (
                      <TableRow className="bg-gray-100 font-bold border-t-2">
                        ...
                        <TableCell className="text-center border-r min-w-[100px] px-4">
                          {data.reduce((sum, item) => sum + item.cancelledOrders, 0)}
                        </TableCell>
